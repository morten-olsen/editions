...
**MCP optional ids:** never branch on `=== undefined` to distinguish create from update. Models send `""`, `null` or `"null"` instead of omitting an optional string, which made creation impossible until fixed. Use `optionalIdSchema` + `resolveOptionalId` from `mcp/mcp.tools.ts`.
