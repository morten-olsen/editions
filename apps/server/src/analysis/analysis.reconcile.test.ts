...
describe("reconcile", () => {
  it("assigns always-mode focuses and classifies match-mode focuses with threshold", async () => {
    ...
    const result = await reconcile(
      ...
      embed,
      strategy,
      { embeddingModel: "test-model", confidenceThreshold: 0.3 },
    );
    ...
    // "Technology" focus (match mode): only AI article scores above 0.3 threshold
    const techAssignments = assignments.filter(
      (a) => a.focus_id === "focus-tech",
    );
    expect(techAssignments).toHaveLength(1);
    expect(techAssignments[0]!.article_id).toBe("art-ai");
    expect(techAssignments[0]!.confidence).toBeCloseTo(0.85, 1);

    // Weather article not assigned to Technology (score 0.1 < threshold 0.3)
    const weatherTech = assignments.find(
      (a) => a.article_id === "art-weather" && a.focus_id === "focus-tech",
    );
    expect(weatherTech).toBeUndefined();
  ...
  it("is idempotent — second run produces no new work", async () => {
    ...
    const { classify } = createFakeClassifier();
    const strategy = createNliStrategy(classify);
    const opts = { embeddingModel: "test-model", confidenceThreshold: 0.3 };
  ...
  it("similarity strategy classifies using embeddings, not NLI", async () => {
    ...
    await reconcile(
      ...
      embed,
      nliStrategy,
      { embeddingModel: "test-model", confidenceThreshold: 0.0 },
    );
