...
class AnalysisService {
  ...
  constructor(services: Services) {
    ...
    this.#confidenceThreshold = analysis.confidenceThreshold;
   ...
   */
  reconcileFocus = async (focusId: string): Promise<ReconcileResult> => {
    const db = await this.#services.get(DatabaseService).getInstance();
  ...
  reanalyseSource = async (
    ...
    // Clear existing classifications so reconcile recomputes them
    await db
      .deleteFrom("article_focuses")
      .where("article_id", "in", articleIds)
      .execute();

    // Reset analysed_at so they get picked up by recovery too
    await db
      .updateTable("articles")
    ...
    for (const article of articles) {
      taskService.enqueue<AnalyseArticlePayload>(
        "analyse_article",
        { articleId: article.id },
        { userId },
      );
