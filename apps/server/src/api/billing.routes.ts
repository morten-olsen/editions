...
type RouteArgs = {
  ...
  requireAdmin: ReturnType<typeof createAdminHook>;
};

// --- Route registration helpers ---

const registerSubscriptionRoutes = ({ fastify, services, authenticate }: RouteArgs): void => {
...
const registerCheckoutRoutes = ({ fastify, services, authenticate }: RouteArgs): void => {
...
const registerWebhookRoute = ({ fastify, services }: RouteArgs): void => {
  // Stripe webhook
  fastify.route({
    method: 'POST',
    url: '/billing/webhook',
...
const registerAdminBillingRoutes = ({ fastify, services, authenticate, requireAdmin }: RouteArgs): void => {
...
const createBillingRoutes =
  ...
  async (fastify) => {
    ...
    const args = { fastify, services, authenticate, requireAdmin };

    registerSubscriptionRoutes(args);
    registerCheckoutRoutes(args);
    registerWebhookRoute(args);
    registerAdminBillingRoutes(args);
  };
