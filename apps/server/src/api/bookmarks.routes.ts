...
const registerListAndCheckRoutes = (
  ...
  fastify.route({
    ...
    schema: {
      ...
      response: { 200: z.object({ bookmarked: z.boolean() }) },
    },
    handler: async (req) => {
      const bookmarksService = services.get(BookmarksService);
  ...
  fastify.route({
    ...
    schema: {
      ...
      response: { 200: z.object({ bookmarkedIds: z.array(z.string()) }) },
    },
    handler: async (req) => {
      const bookmarksService = services.get(BookmarksService);
...
const createBookmarksRoutes =
  ...
  async (fastify) => {
    ...
    registerListAndCheckRoutes(fastify, services, authenticate);
    registerMutationRoutes(fastify, services, authenticate);
  };
