...
const up = async (db: Kysely<unknown>): Promise<void> => {
  ...
  await db.schema
    .createIndex("idx_sources_user_id")
    .on("sources")
    .column("user_id")
    .execute();
  ...
  await db.schema
    .createIndex("idx_articles_source_id")
    .on("articles")
    .column("source_id")
    .execute();
  ...
  await db.schema
    .createIndex("idx_articles_published_at")
    .on("articles")
    .column("published_at")
    .execute();
  ...
  await db.schema
    .createIndex("idx_focuses_user_id")
    .on("focuses")
    .column("user_id")
    .execute();
  ...
  await db.schema
    ...
    .addColumn("focus_id", "text", (col) =>
      col.notNull().references("focuses.id").onDelete("cascade"),
    )
    .addColumn("source_id", "text", (col) =>
      col.notNull().references("sources.id").onDelete("cascade"),
    )
  ...
  await db.schema
    .createIndex("idx_focus_sources_source_id")
    .on("focus_sources")
    .column("source_id")
    .execute();
  ...
  await db.schema
    ...
    .addColumn("assigned_at", "text", (col) =>
      col.notNull().defaultTo(sql`(datetime('now'))`),
    )
    .execute();
  ...
  await db.schema
    .createIndex("idx_article_focuses_focus_id")
    .on("article_focuses")
    .column("focus_id")
    .execute();
  ...
  await db.schema
    ...
    .addColumn("article_id", "text", (col) =>
      col.primaryKey().references("articles.id").onDelete("cascade"),
    )
  ...
  await db.schema
    ...
    .addColumn("exclude_prior_editions", "integer", (col) =>
      col.notNull().defaultTo(0),
    ) // boolean — skip articles already in prior editions of this config
  ...
  await db.schema
    .createIndex("idx_edition_configs_user_id")
    .on("edition_configs")
    .column("user_id")
    .execute();
  ...
  await db.schema
    ...
    .addColumn("edition_config_id", "text", (col) =>
      col.notNull().references("edition_configs.id").onDelete("cascade"),
    )
    .addColumn("focus_id", "text", (col) =>
      col.notNull().references("focuses.id").onDelete("cascade"),
    )
  ...
  await db.schema
    ...
    .addColumn("current_position", "integer", (col) =>
      col.notNull().defaultTo(0),
    ) // reading progress — last article position reached
    .addColumn("published_at", "text", (col) =>
      col.notNull().defaultTo(sql`(datetime('now'))`),
    )
    .addColumn("created_at", "text", (col) =>
      col.notNull().defaultTo(sql`(datetime('now'))`),
  ...
  await db.schema
    .createIndex("idx_editions_config_id")
    .on("editions")
    .column("edition_config_id")
    .execute();
  ...
  await db.schema
    ...
    .addColumn("edition_id", "text", (col) =>
      col.notNull().references("editions.id").onDelete("cascade"),
    )
    .addColumn("article_id", "text", (col) =>
      col.notNull().references("articles.id").onDelete("cascade"),
    )
