...
const up = async (db: Kysely<unknown>): Promise<void> => {
  await db.schema
    ...
    .addColumn('focus_id', 'text', (col) => col.references('focuses.id').onDelete('cascade'))
  ...
  await db.schema.createIndex('idx_article_votes_user_id').on('article_votes').column('user_id').execute();
};
