...
const up = async (db: Kysely<unknown>): Promise<void> => {
  ...
  await db.schema.alterTable('articles').addColumn('consumption_time_seconds', 'integer').execute();
  ...
  await db.schema.alterTable('articles').addColumn('media_url', 'text').execute();
  await db.schema.alterTable('articles').addColumn('media_type', 'text').execute();
  ...
  await db.schema.alterTable('focuses').addColumn('min_consumption_time_seconds', 'integer').execute();
  ...
  await db.schema.alterTable('focuses').addColumn('max_consumption_time_seconds', 'integer').execute();
...
const down = async (db: Kysely<unknown>): Promise<void> => {
  ...
  await db.schema.alterTable('focuses').addColumn('max_reading_time_seconds', 'integer').execute();
  ...
  await db.schema.alterTable('focuses').addColumn('min_reading_time_seconds', 'integer').execute();
  ...
  await db.schema.alterTable('articles').addColumn('word_count', 'integer').execute();
  ...
  await db.schema.alterTable('articles').addColumn('reading_time_seconds', 'integer').execute();
