...
class FocusesService {
  ...
  list = async (userId: string): Promise<Focus[]> => {
    ...
    for (const link of sourceLinks) {
      ...
      arr.push({ sourceId: link.source_id, weight: link.weight, minConfidence: link.min_confidence });
  ...
  create = async (params: CreateFocusParams): Promise<Focus> => {
    ...
    // Always reconcile — score all existing articles against this new focus
  ...
  setSources = async (userId: string, focusId: string, sources: FocusSource[]): Promise<Focus> => {
    ...
    // Replace all source associations — no reconcile needed since all articles
    // are already scored against all focuses. focus_sources only controls
    // which articles are shown (threshold filtering), not which are scored.
