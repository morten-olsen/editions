...
class JobService {
  ...
  listByUser = (userId: string, filter?: ListJobsFilter): Job[] => {
    ...
    for (const job of this.#jobs.values()) {
      ...
      if (filter?.sourceId) {
        const matches = job.affects.sourceIds.length === 0 || job.affects.sourceIds.includes(filter.sourceId);
        if (!matches) {
          continue;
        }
      }
      ...
      if (filter?.focusId) {
        const matches = job.affects.focusIds.length === 0 || job.affects.focusIds.includes(filter.focusId);
        if (!matches) {
          continue;
        }
      }
  ...
  #run = async (job: Job, handler: JobHandler): Promise<void> => {
    try {
      ...
      job.status = 'failed';
      job.error = err instanceof Error ? [err.message, err.stack].filter(Boolean).join('\n') : String(err);
    }
