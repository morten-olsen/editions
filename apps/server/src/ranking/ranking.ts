import { sql } from 'kysely';
import type { RawBuilder, SqlBool } from 'kysely';

// --- Types ---

type ScoringWeights = {
  alpha: number; // confidence weight
  beta: number; // vote propagation weight
  gamma: number; // recency decay weight
};

type UserScoringWeights = {
  global: ScoringWeights;
  focus: ScoringWeights;
  edition: ScoringWeights;
};

type VotedArticle = {
  embedding: Float32Array;
  value: 1 | -1;
};

type VoteContext = {
  votes: Map<string, 1 | -1>;
  votedArticles: VotedArticle[];
};

type RankingCandidate = {
  articleId: string;
  sourceId?: string;
  similarity: number | null;
  nli: number | null;
  publishedAt: string | null;
  // Raw embedding as stored (BLOB Buffer), already-decoded Float32Array, or null.
  // This module owns the wire format — callers never decode.
  embedding: unknown;
};

type Ranked<T> = {
  item: T;
  score: number;
};

type ScoreAndRankParams<T extends RankingCandidate> = {
  candidates: T[];
  voteContext: VoteContext;
  weights: ScoringWeights;
  sourceWeights?: ReadonlyMap<string, number>;
  focusWeight?: number;
  now?: number;
};

type MinConfidenceFilterParams = {
  minConfidence: number;
  perSource?: ReadonlyMap<string, number>;
};

// --- Constants ---

const RECENCY_HALF_LIFE_DAYS = 3;
const MAX_VOTE_CONTEXT_SIZE = 200;

// Top-k propagation: only the k most similar voted articles contribute
const PROPAGATION_TOP_K = 15;
const PROPAGATION_MIN_SIMILARITY = 0.4;

// Minimum number of votes before vote signal reaches full weight.
// With fewer votes, beta is scaled down proportionally to avoid
// over-weighting sparse signal.
const VOTE_RAMP_THRESHOLD = 5;

// Per-feed-type weight presets
const globalWeights: ScoringWeights = { alpha: 0, beta: 0.6, gamma: 0.4 };
const focusWeights: ScoringWeights = { alpha: 0.4, beta: 0.4, gamma: 0.2 };
const editionWeights: ScoringWeights = { alpha: 0.5, beta: 0.4, gamma: 0.1 };

const defaultUserScoringWeights: UserScoringWeights = {
  global: globalWeights,
  focus: focusWeights,
  edition: editionWeights,
};

// --- Private helpers ---

const cosineSimilarity = (a: Float32Array, b: Float32Array): number => {
  let dot = 0;
  for (let i = 0; i < a.length; i++) {
    dot += (a[i] as number) * (b[i] as number);
  }
  // Embeddings are L2-normalized, so dot product = cosine similarity
  return dot;
};

const recencyDecay = (publishedAt: string | null, now: number): number => {
  if (!publishedAt) {
    return 0.5;
  }
  const daysSince = (now - new Date(publishedAt).getTime()) / (1000 * 60 * 60 * 24);
  return Math.pow(0.5, daysSince / RECENCY_HALF_LIFE_DAYS);
};

const computeScore = (params: {
  candidate: RankingCandidate;
  embedding: Float32Array | null;
  context: VoteContext;
  weights: ScoringWeights;
  now: number;
}): number => {
  const { candidate, embedding, context, weights, now } = params;
  const directVote = context.votes.get(candidate.articleId);

  let voteSignal: number;
  if (directVote !== undefined) {
    // Direct vote replaces propagated score — no double-counting
    voteSignal = directVote;
  } else if (embedding && context.votedArticles.length > 0) {
    // Top-k propagation: only the most similar voted articles contribute,
    // weighted by similarity so nearby votes matter more
    const scored: { sim: number; value: 1 | -1 }[] = [];
    for (const voted of context.votedArticles) {
      const sim = cosineSimilarity(embedding, voted.embedding);
      if (sim >= PROPAGATION_MIN_SIMILARITY) {
        scored.push({ sim, value: voted.value });
      }
    }
    if (scored.length > 0) {
      scored.sort((a, b) => b.sim - a.sim);
      const topK = scored.slice(0, PROPAGATION_TOP_K);
      let weightedSum = 0;
      let simSum = 0;
      for (const s of topK) {
        weightedSum += s.value * s.sim;
        simSum += s.sim;
      }
      voteSignal = weightedSum / simSum;
    } else {
      voteSignal = 0;
    }
  } else {
    voteSignal = 0;
  }

  const recency = recencyDecay(candidate.publishedAt, now);

  // Scale vote weight by how many votes exist — avoids over-weighting sparse signal
  const voteCount = context.votes.size + context.votedArticles.length;
  const voteRamp = Math.min(1, voteCount / VOTE_RAMP_THRESHOLD);
  const effectiveBeta = weights.beta * voteRamp;
  const effectiveAlpha = weights.alpha + weights.beta * (1 - voteRamp);

  return effectiveAlpha * effectiveConfidence(candidate) + effectiveBeta * voteSignal + weights.gamma * recency;
};

// --- Public functions ---

// The stored embedding wire format: float32 array serialized as a BLOB.
const decodeEmbedding = (value: unknown): Float32Array | null => {
  if (value instanceof Float32Array) {
    return value;
  }
  if (value instanceof Uint8Array) {
    return new Float32Array(value.buffer, value.byteOffset, value.byteLength / 4);
  }
  return null;
};

const effectiveConfidence = (c: { similarity: number | null; nli: number | null }): number =>
  c.nli ?? c.similarity ?? 0;

// SQL half of the confidence rule — must stay in sync with effectiveConfidence
const effectiveConfidenceSql = (): RawBuilder<number> =>
  sql<number>`COALESCE(article_focuses.nli, article_focuses.similarity)`;

// Builds the WHERE expression enforcing a focus's confidence threshold,
// with optional per-source overrides. Returns null when no filtering applies.
const minConfidenceFilterSql = ({
  minConfidence,
  perSource,
}: MinConfidenceFilterParams): RawBuilder<SqlBool> | null => {
  const cases = [...(perSource?.entries() ?? [])].map(
    ([sourceId, minConf]) => sql`WHEN articles.source_id = ${sourceId} THEN ${minConf}`,
  );

  if (minConfidence <= 0 && cases.length === 0) {
    return null;
  }

  const thresholdExpr =
    cases.length > 0 ? sql`CASE ${sql.join(cases, sql` `)} ELSE ${minConfidence} END` : sql`${minConfidence}`;

  return sql<SqlBool>`${effectiveConfidenceSql()} >= ${thresholdExpr}`;
};

// Score and rank candidates, highest first. Each candidate is scored exactly once:
// score = computeScore(candidate) × sourceWeight × focusWeight
const scoreAndRank = <T extends RankingCandidate>(params: ScoreAndRankParams<T>): Ranked<T>[] => {
  const { candidates, voteContext, weights, sourceWeights, focusWeight = 1, now = Date.now() } = params;

  const scored = candidates.map((c) => {
    const sourceWeight = (c.sourceId !== undefined ? sourceWeights?.get(c.sourceId) : undefined) ?? 1;
    const base = computeScore({
      candidate: c,
      embedding: decodeEmbedding(c.embedding),
      context: voteContext,
      weights,
      now,
    });
    return { item: c, score: base * sourceWeight * focusWeight };
  });

  scored.sort((a, b) => b.score - a.score);
  return scored;
};

const mergeVoteContexts = (global: VoteContext, focusScoped: VoteContext): VoteContext => {
  // Focus-scoped votes take precedence over global for the same article
  const votes = new Map(global.votes);
  for (const [articleId, value] of focusScoped.votes) {
    votes.set(articleId, value);
  }

  // Concatenate voted articles — duplicates average out in propagation
  const votedArticles = [...global.votedArticles, ...focusScoped.votedArticles];

  return { votes, votedArticles };
};

const emptyVoteContext = (): VoteContext => ({
  votes: new Map(),
  votedArticles: [],
});

const parseUserScoringWeights = (json: string | null): UserScoringWeights => {
  if (!json) {
    return defaultUserScoringWeights;
  }
  try {
    const parsed = JSON.parse(json) as Partial<UserScoringWeights>;
    return {
      global: { ...globalWeights, ...parsed.global },
      focus: { ...focusWeights, ...parsed.focus },
      edition: { ...editionWeights, ...parsed.edition },
    };
  } catch {
    return defaultUserScoringWeights;
  }
};

export type { ScoringWeights, UserScoringWeights, VotedArticle, VoteContext, RankingCandidate, Ranked };
export {
  scoreAndRank,
  effectiveConfidence,
  effectiveConfidenceSql,
  minConfidenceFilterSql,
  decodeEmbedding,
  mergeVoteContexts,
  emptyVoteContext,
  defaultUserScoringWeights,
  parseUserScoringWeights,
  MAX_VOTE_CONTEXT_SIZE,
};
