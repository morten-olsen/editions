...
const queryPage = (depth: number = 1): AiPageDescriptor => {
...
const queryElement = (id: string, depth: number = 2): AiNode | null => {
  const el = document.querySelector(`[data-ai-id="${CSS.escape(id)}"]`);
  if (!el) return null;
  return readNode(el, 1, depth);
};

const getElementHtml = (id: string, maxLength: number = 2000): string | null => {
  const el = document.querySelector(`[data-ai-id="${CSS.escape(id)}"]`);
  if (!el) return null;
