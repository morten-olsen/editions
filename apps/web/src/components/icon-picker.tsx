...
const IconPicker = ({ value, onChange }: IconPickerProps): React.ReactElement => {
  ...
  const filtered = useMemo(() => {
    ...
    return [...matches.filter((n) => curated.has(n)), ...matches.filter((n) => !curated.has(n))];
  }, [search]);

  return (
    <div className="flex flex-col gap-1.5">
      <label className="text-sm font-medium text-ink">Icon</label>
      <p className="text-xs text-ink-tertiary -mt-0.5">Optional — shown in the sidebar</p>
      ...
      <div className="grid grid-cols-8 gap-1 max-h-48 overflow-y-auto rounded-md border border-border bg-surface p-2">
        ...
        {filtered.length === 0 && (
          <div className="col-span-8 text-xs text-ink-tertiary py-3 text-center">No icons found</div>
        )}
      </div>
