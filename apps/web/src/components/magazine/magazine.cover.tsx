...
const MagazineCover = ({
  ...
  return (
    <MagazinePage className="relative justify-between min-h-screen !p-0 overflow-hidden">
      ...
      {/* ── Content layer ───────────────────────────────────────────── */}
      <div
        className={`relative z-10 flex flex-col justify-between min-h-screen px-6 py-12 md:px-12 lg:px-20 ${lead.imageUrl ? 'text-white' : ''}`}
      >
        ...
        >
          <div className="flex items-baseline gap-3">
            <span
              className={`text-xs font-mono tracking-wide uppercase ${lead.imageUrl ? 'text-white/90' : 'text-accent'}`}
            >
              Editions
            </span>
            <span className={`text-xs ${lead.imageUrl ? 'text-white/40' : 'text-ink-faint'}`}>/</span>
            <span
              className={`text-xs tracking-wide uppercase ${lead.imageUrl ? 'text-white/70' : 'text-ink-tertiary'}`}
            >
              {editionTitle}
            </span>
          </div>
        ...
        >
          ...
          <h1
            className={`font-serif text-5xl md:text-[4rem] lg:text-[5rem] leading-none tracking-tight mb-6 ${lead.imageUrl ? 'text-white' : 'text-ink'}`}
          >
            {lead.title}
          </h1>
        ...
        >
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
                ...
                {highlights.map((h, i) => (
                  <div key={i} className="flex gap-3 max-w-80">
                    ...
                    <div className="min-w-0">
                      <div
                        className={`text-xs font-mono tracking-wide mb-1 ${lead.imageUrl ? 'text-white/50' : 'text-ink-faint'}`}
                      >
                        {h.sourceName}
                      </div>
                      <div
                        className={`font-serif text-sm leading-snug line-clamp-2 ${lead.imageUrl ? 'text-white/90' : 'text-ink'}`}
                      >
                        {h.title}
                      </div>
                    </div>
            ...
            {/* Edition stats */}
            <div
              className={`flex gap-6 text-xs font-mono tracking-wide shrink-0 ${lead.imageUrl ? 'text-white/50' : 'text-ink-tertiary'}`}
            >
