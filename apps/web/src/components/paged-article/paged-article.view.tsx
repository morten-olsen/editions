...
const useKeyboardPaging = ({ goNext, goPrev, goTo, total }: PagingHandlers): void => {
  useEffect(() => {
...
const useTouchPaging = (
  ...
  useEffect(() => {
    ...
    const onTouchEnd = (e: TouchEvent): void => {
      ...
      if (e.changedTouches.length !== 1 || !t) {
        return;
      }
...
const PagedArticleView = ({
  ...
  const result = useMemo(() => {
    if (!fontsReady || debouncedSize.width === 0) {
      return null;
    }
  ...
  useEffect(() => {
    if (result) {
      onPageChange?.(page, result.pageCount);
    }
  ...
  // Keyboard navigation
  useKeyboardPaging({ goNext, goPrev, goTo, total });

  // Touch: swipe + edge tap
  useTouchPaging(containerRef, goNext, goPrev);
  ...
  if (!fontsReady || !result) {
    return (
      <div ref={containerRef} className="w-full h-full flex items-center justify-center bg-surface">
        <div className="text-xs font-mono text-ink-tertiary tracking-wide animate-pulse">Preparing layout&hellip;</div>
      </div>
    );
  }
  ...
  return (
    <div ref={containerRef} className="relative bg-surface w-full h-full overflow-hidden select-none">
      <AnimatePresence mode="wait" initial={false}>
        ...
        >
          <PageRenderer page={currentPage} config={result.config} footer={footer} isLastPage={isLastPage} />
        </motion.div>
      </AnimatePresence>
