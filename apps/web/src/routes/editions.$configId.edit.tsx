...
const EditEditionConfigPage = (): React.ReactNode => {
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    if (focusesChanged) {
      body.focuses = selectedFocuses.map((f, i) => ({ ...f, position: i }));
    }
          ...
          {selectedFocuses.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {selectedFocuses.map((focusConfig, idx) => {
                ...
                return (
                  <div key={focusConfig.focusId} className="border border-border rounded-md p-3">
                    ...
                    <div className="mt-2 flex items-center gap-3 pl-8">
                      <select
                        ...
                        onChange={(e) =>
                          updateFocusBudget(focusConfig.focusId, "budgetType", e.target.value)
                        }
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateFocusBudget(focusConfig.focusId, "budgetValue", Number(e.target.value))
                        }
                      ...
                      <span className="text-xs text-ink-tertiary">
                        {focusConfig.budgetType === "count" ? "articles" : "min"}
                      </span>
                    </div>
                  </div>
                );
