    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid gap-3 text-xs text-ink-secondary leading-relaxed">
          ...
          <div className="flex gap-3">
            ...
            <span>
              ...
              propagates to semantically similar articles via embeddings — so a few votes shape the ranking of hundreds
              of articles. Only the 15 most similar voted articles contribute, weighted by how similar they are.
            </span>
          </div>
          <div className="flex gap-3">
        ...
        <p className="text-sm text-ink-secondary leading-relaxed">
          The Greek letters (α, β, γ) are the weights you control below. They determine how much each signal matters.
          Each feed type has its own set of weights because the context is different — for example, the global feed has
          no focus topic, so confidence is irrelevant there.
        </p>
        <p className="text-sm text-ink-secondary leading-relaxed">
          The weights don't need to sum to 1. Higher values amplify a signal relative to the others; setting a weight to
          0 disables that signal entirely.
        </p>
      </div>
...
const SettingsPage = (): React.ReactNode => {
  ...
  const handleFilterChange = (newScope: ScopeFilter = scopeFilter, newValue: ValueFilter = valueFilter): void => {
      ...
      >
        {tabs.map((tab) => (
          ...
          >
            {tab.label}
            {tab.badge && (
              <span className="ml-1.5 text-[10px] font-medium text-accent/60 uppercase tracking-wider">
                {tab.badge}
              </span>
            )}
          </button>
                ...
                {votesPage.votes.map((vote, idx) => (
                  <div key={vote.id}>
                    ...
                    <div className="flex items-start gap-4 py-4">
                      <div
                        className={`mt-0.5 shrink-0 w-5 text-center text-sm ${vote.value === 1 ? 'text-accent' : 'text-critical'}`}
                      >
                        {vote.value === 1 ? '\u2191' : '\u2193'}
                      </div>
