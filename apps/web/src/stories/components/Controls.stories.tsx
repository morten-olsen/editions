...
const SeparatorStory: Story = {
  render: () => (
    <div style={{ width: '24rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div style={{ fontSize: '0.875rem', color: 'var(--color-ink-secondary)' }}>Content above</div>
      <Separator />
      <div style={{ fontSize: '0.875rem', color: 'var(--color-ink-secondary)' }}>Default separator</div>
      <Separator soft />
      <div style={{ fontSize: '0.875rem', color: 'var(--color-ink-secondary)' }}>Soft separator (lighter)</div>
    </div>
  ),
  name: 'Separator',
};
