...
type HomeEdition = {
  ...
  sections: Array<{ focusName: string; articleCount: number }>;
  lead: {
    title: string;
    sourceName: string;
  ...
  highlights: Array<{ title: string; sourceName: string }>;
};
...
const sampleEditions: HomeEdition[] = [
  ...
  },
  {
    ...
    highlights: [
      { title: "Why SQLite is the database you didn't know you needed", sourceName: "Hacker News" },
    ],
  },
  {
    id: "ed-3",
    ...
    highlights: [
      { title: "TypeScript 6.0 introduces pattern matching", sourceName: "Hacker News" },
    ],
  },
];
    ...
    <div className="group block rounded-lg overflow-hidden relative isolate cursor-pointer">
      ...
      <div className={`flex flex-col justify-between p-5 min-h-56 ${hasImage ? "text-white" : ""}`}>
        <div className="flex items-baseline justify-between gap-3">
          <span className={`font-mono text-xs tracking-wide uppercase ${hasImage ? "text-white/80" : "text-accent"}`}>{edition.configName}</span>
          <span className={`font-mono text-xs tracking-wide ${hasImage ? "text-white/50" : "text-ink-faint"}`}>{formatPubDate(edition.publishedAt)}</span>
        </div>
        <div className="mt-auto pt-4">
          {edition.lead && (
            <>
              <div className={`font-mono text-xs tracking-wide mb-2 ${hasImage ? "text-white/60" : "text-ink-faint"}`}>{edition.lead.sourceName}</div>
              <h3 className={`font-serif text-xl md:text-2xl font-medium tracking-tight leading-snug mb-3 ${hasImage ? "text-white" : "text-ink"}`}>{edition.lead.title}</h3>
            </>
          )}
          {edition.sections.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-3">
              {edition.sections.map((s) => (
                <span key={s.focusName} className={`text-xs px-2 py-0.5 rounded-full ${hasImage ? "bg-white/15 text-white/80" : "bg-surface-sunken text-ink-tertiary"}`}>{s.focusName}</span>
              ))}
            </div>
          )}
...
const SetupStepStatic = ({ number, title, description, done, active, actionLabel }: SetupStepStaticProps): React.ReactElement => (
    ...
    <div className="flex items-start gap-5">
      <div className="shrink-0 w-7 pt-0.5">
        {done ? (
          ...
          <span className={`font-mono text-sm ${active ? "text-ink-faint" : "text-ink-faint/40"}`}>{String(number).padStart(2, "0")}</span>
        )}
      </div>
      <div className="flex-1 min-w-0">
        <div className={`font-serif text-lg font-medium tracking-tight ${done ? "text-ink-tertiary" : active ? "text-ink" : "text-ink-faint"}`}>{title}</div>
        <div className={`text-sm mt-1 leading-relaxed ${done || active ? "text-ink-tertiary" : "text-ink-faint"}`}>{description}</div>
        {!done && active && (
          <div className="mt-4"><Button variant="primary" size="sm">{actionLabel}</Button></div>
        )}
      </div>
    </div>
  </div>
...
const QuickLinks = ({ configs }: { configs: HomeConfig[] }): React.ReactElement => (
  <div className="mt-8 flex flex-wrap items-center gap-x-5 gap-y-2">
    <span className="font-mono text-xs tracking-wide text-ink-tertiary hover:text-ink transition-colors duration-fast cursor-pointer">Browse feed</span>
    <span className="font-mono text-xs tracking-wide text-ink-tertiary hover:text-ink transition-colors duration-fast cursor-pointer">Bookmarks</span>
    {configs.map((cfg) => (
      <span key={cfg.id} className="font-mono text-xs tracking-wide text-ink-tertiary hover:text-ink transition-colors duration-fast cursor-pointer">{cfg.name}</span>
    ))}
  </div>
);
  ...
  render: () => (
    <ContentShell>
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-ink-secondary leading-relaxed">A finite, curated reading experience. Set up your sources, define your interests, and create your first edition — a magazine built just for you.</p>
      </div>
      <div>
        <SetupStepStatic number={1} title="Add sources" description="Subscribe to RSS feeds and podcasts. These are the raw ingredients for your editions." done={false} active={true} actionLabel="Add your first source" />
        <SetupStepStatic number={2} title="Create focuses" description="Define topic areas like Technology, Science, or Local News. Articles are automatically classified into your focuses." done={false} active={false} actionLabel="Create a focus" />
        <SetupStepStatic number={3} title="Create an edition" description="Build a scheduled, finite digest from your focuses. Set time budgets, pick sections, and read on your terms." done={false} active={false} actionLabel="Create your first edition" />
...
const PartialSetup: Story = {
  render: () => (
    <ContentShell>
      ...
      <div>
        <SetupStepStatic number={1} title="Add sources" description="Subscribe to RSS feeds and podcasts. These are the raw ingredients for your editions." done={true} active={false} actionLabel="Add your first source" />
        <SetupStepStatic number={2} title="Create focuses" description="Define topic areas like Technology, Science, or Local News. Articles are automatically classified into your focuses." done={false} active={true} actionLabel="Create a focus" />
        <SetupStepStatic number={3} title="Create an edition" description="Build a scheduled, finite digest from your focuses. Set time budgets, pick sections, and read on your terms." done={false} active={false} actionLabel="Create your first edition" />
        <div className="h-px bg-border" />
      </div>
      <div className="mt-6">
  ...
  render: () => (
    <ContentShell>
      ...
      <div className="py-12 text-center">
        ...
        <p className="text-sm text-ink-tertiary leading-relaxed">You're up to date. New editions will appear here when they're generated.</p>
      </div>
      <QuickLinks configs={sampleConfigs} />
    </ContentShell>
  ),
