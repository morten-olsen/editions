  ...
  <div className="grid grid-cols-5 gap-6">
    {shadows.map((shadow) => (
      <div key={shadow.name} className="text-center">
        <div className={`${shadow.className} bg-surface-overlay rounded-lg h-24 mb-4 border border-border/50`} />
        <div className="text-sm font-medium text-ink">{shadow.name}</div>
        <div className="text-xs font-mono text-ink-tertiary mt-0.5">shadow-{shadow.name}</div>
...
const ShadowElevation = (): React.ReactElement => (
  <div className="relative bg-surface-sunken rounded-xl p-12 flex flex-col gap-8">
    <div className="text-xs font-mono text-ink-tertiary tracking-wide uppercase mb-8">Layered elevation</div>

    {/* Base surface */}
    <div className="bg-surface rounded-lg p-8 border border-border">
      <div className="text-sm text-ink-secondary mb-6">Base surface — flat, no shadow. Most content lives here.</div>
      ...
      <div className="bg-surface-raised rounded-lg shadow-sm p-6 border border-border/50 max-w-md">
        ...
        <div className="font-medium text-ink mb-1">The quiet revolution in reader design</div>
        <div className="text-sm text-ink-secondary">A raised card with subtle shadow — interactive, tappable.</div>
      </div>
    </div>
