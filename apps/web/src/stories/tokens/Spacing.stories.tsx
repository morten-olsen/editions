  ...
  <div className="flex flex-col gap-3">
    {spacingSteps.map((step) => (
      <div key={step.name} className="flex items-center gap-4">
        <span className="text-xs font-mono text-ink-tertiary w-8 text-right shrink-0">
          {step.name}
        </span>
        <div
...
const SpacingCompositions = (): React.ReactElement => (
  <div className="grid grid-cols-2 gap-8">
    ...
    <div className="border border-border rounded-lg p-8">
      ...
      <div>
        <div className="text-xs text-accent tracking-wide uppercase font-medium mb-2">
          Technology
        </div>
        <div className="text-2xl tracking-tight text-ink leading-tight mb-3">
          Your morning briefing
        </div>
        <div className="text-sm text-ink-secondary">
          5 articles · approximately 12 minutes
        </div>
      </div>
    </div>
